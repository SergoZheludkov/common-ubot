const locale = {
  ...
  wallets: {
    ...
    add_wallets_success: 'Успешно добавленны и запущенны в ротацию кошельки в количестве',
    add_wallets_failed:
    ...
    deactivate_message: 'Выбери кошельки для деактивации',

    deactivate_wallets_success: 'Успешно деактивированны кошельки в количестве',
    deactivate_wallets_failed:
      'Произошла ошабка при деактивации кошельков на сервере. Если ты не разработчик - срочно отпиши ему о проблеме',
  },
  rules: {
    message:
      'Правила:\n\n‼️ЗАМЕНА НЕВАЛИДА производится во вкладке поддержка!!!\n⏳Заявки рассматриваются в течении дня!\n‼️Заменяем 100% если не отображается линк до входа либо товар не соответствует описанию\n\n⚡️Задать любой вопрос вы можете нажав кнопку «помощь» в боте, создав соответсвующий тикет.',
  },
};
